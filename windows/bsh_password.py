...
class BSHPasswordAuth:
    ...
    def save_passwords(self) -> None:
        ...
        # Bug #4 fix: os.chmod(0o600) on Windows only sets the Read-Only attribute,
        # not POSIX permissions.  Use win32security to apply a real DACL that grants
        # access only to SYSTEM and the Administrators group.
        try:
            import win32security, ntsecuritycon as con
            sd  = win32security.GetFileSecurity(
                str(self.password_file), win32security.DACL_SECURITY_INFORMATION
            )
            dacl = win32security.ACL()
            for sid_str in ('S-1-5-18', 'S-1-5-32-544'):  # SYSTEM, Administrators
                sid = win32security.CreateWellKnownSid(
                    win32security.WinLocalSystemSid
                    if sid_str == 'S-1-5-18'
                    else win32security.WinBuiltinAdministratorsSid,
                    None,
                )
            ...
            win32security.SetFileSecurity(
                str(self.password_file), win32security.DACL_SECURITY_INFORMATION, sd
            )
        except ImportError:
            # win32security unavailable — fall back to POSIX chmod on non-Windows
            try:
                os.chmod(self.password_file, 0o600)
            except AttributeError:
                pass  # Truly no chmod support
        except Exception:
            pass  # Best-effort; log would require an injected logger here
...
def main() -> int:
    ...
    _pf = dict(
        ...
        kwargs=dict(default='~/.bsh/passwords', metavar='FILE',
                    help='Password database (default: ~/.bsh/passwords)'),
    )

    # adduser
    ap = sub.add_parser('adduser', help='Add a BSH user')
    ap.add_argument('username')
    ap.add_argument('-s', '--system-user', help='OS username')
    ...
    if args.command == 'adduser':
        ...
        try:
            ...
            print(f"Warning: system user '{system_user}' not found or not on POSIX.")
